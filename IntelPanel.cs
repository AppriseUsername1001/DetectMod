...
internal sealed class IntelPanel : NativeChildForm
{
	...
	/// <summary>ControlBarForm이 두 패널을 다 만든 뒤 연결해준다 — 캐릭터 목록 UI는
	/// ZkbFeedPanel이 소유하므로, 로그인/로그아웃 흐름(이 클래스 소관)이 그쪽에 새로고침을
	/// 요청하거나 반대로 "전부 로그아웃됨" 알림을 받으려면 서로를 참조해야 한다.</summary>
	[System.ComponentModel.DesignerSerializationVisibility(System.ComponentModel.DesignerSerializationVisibility.Hidden)]
	public ZkbFeedPanel? ZkbPanel { get; set; }

	public void ShowLoginScreen() => ShowLogin();
	public void RequestAddCharacter() => _ = DoLoginAsync();
	...
	public void SetVisible(bool visible)
	{
		...
		if (visible)
		{
			...
			if (!IsShownInHost)
				ReparentToHost();
			if (_settings.IntelCharacters.Count > 0)
				_engine.Start();
			try { _logList?.ForceRepaint(); } catch { }
		}
		else
	...
	private void BuildDashboard()
	{
		...
		{
			var main = _settings.GetMainCharacter();
			if (main is not null) ZkbPanel?.RemoveCharacter(main);
		};
